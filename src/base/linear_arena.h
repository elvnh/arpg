...
#define la_allocate_array(arena, type, count)                                            \
    (type *)((la_allocate(arena, count, sizeof(type), ALIGNOF(type))))
#define la_allocate_item(arena, type) la_allocate_array(arena, type, 1)
#define la_copy_array(arena, ptr, count)                                                 \
    la_copy_allocation((arena), (ptr), (count), sizeof(*(ptr)), ALIGNOF(*(ptr)))
