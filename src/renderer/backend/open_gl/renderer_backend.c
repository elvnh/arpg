...
#define VERTEX_SHADER_DIRECTIVE   str("#vertex")
#define FRAGMENT_SHADER_DIRECTIVE str("#fragment")

#define GLOBAL_UNIFORMS_BINDING_POINT 0
#define GLOBAL_UNIFORMS_NAME          "GlobalUniforms"

#define GLSL_VERSION_STRING "#version 330\n"
#define GLOBAL_UNIFORMS_SOURCE                                                           \
    "layout (std140) uniform " GLOBAL_UNIFORMS_NAME " { "                                \
    "    uniform mat4 u_proj; "                                                          \
    "}; "
...
RendererBackend *renderer_backend_initialize(Vector2i window_dims, Allocator allocator)
{
    ...
    glVertexAttribPointer(POSITION_ATTRIBUTE, 2, GL_FLOAT, GL_FALSE, stride,
    ...
    glBindBufferRange(GL_UNIFORM_BUFFER, GLOBAL_UNIFORMS_BINDING_POINT, state->ubo, 0,
        sizeof(GlobalShaderUniforms));
...
static SplitShaderSource split_shader_source(String source)
{
    ...
    String fragment_src =
        str_create_span(source, fragment_index, fragment_end - fragment_index);
...
TextureAsset *renderer_backend_create_texture(Image image, Allocator allocator)
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, image.channels, image.width, image.height, 0, channel,
...
void renderer_backend_set_global_projection(RendererBackend *backend, Matrix4 matrix)
{
    ...
    glBufferSubData(GL_UNIFORM_BUFFER, offsetof(GlobalShaderUniforms, projection),
        sizeof(Matrix4), matrix.data);
