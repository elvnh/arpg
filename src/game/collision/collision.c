...
static void rect_collision_reset_velocities(
    Vector2 *a, Vector2 *b, CardinalDirection side)
{
...
CollisionInfo collision_rect_vs_rect(f32 movement_fraction_left, Rectangle rect_a,
    Rectangle rect_b, Vector2 velocity_a, Vector2 velocity_b, f32 dt)
{
    ASSERT(movement_fraction_left >= 0.0f);

    CollisionInfo result = {.new_position_a = rect_a.position,
    ...
    if (rect_contains_point(md, V2_ZERO)
        && (v2_mag(penetration.point) > INTERSECTION_EPSILON)) {
        ...
        result.new_position_a =
            v2_add(new_pos, v2_mul_s(collision_normal, COLLISION_MARGIN));
        result.collision_status = COLLISION_STATUS_ARE_INTERSECTING;
        result.collision_normal = collision_normal;

        rect_collision_reset_velocities(
            &result.new_velocity_a, &result.new_velocity_b, penetration.side);
    } else {
        ...
        RectRayIntersection intersection =
            rect_shortest_ray_intersection(md, ray_line, INTERSECTION_EPSILON);

        if (intersection.is_intersecting) {
            Vector2 a_dist_to_move =
                v2_mul_s(velocity_a, intersection.time_of_impact * dt);
            Vector2 b_dist_to_move =
                v2_mul_s(velocity_b, intersection.time_of_impact * dt);
            ...
            result.new_position_a = v2_add(
                result.new_position_a, v2_mul_s(v2_norm(velocity_a), -COLLISION_MARGIN));
            result.new_position_b = v2_add(
                result.new_position_b, v2_mul_s(v2_norm(velocity_b), -COLLISION_MARGIN));

            rect_collision_reset_velocities(&result.new_velocity_a,
                &result.new_velocity_b, intersection.side_of_collision);
