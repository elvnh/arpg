...
#define fl_alloc_array(arena, type, count)                                               \
    ((type *)(fl_allocate(arena, count, SIZEOF(type), ALIGNOF(type))))
#define fl_alloc_item(arena, type) fl_alloc_array(arena, type, 1)
#define fl_realloc_array(arena, type, ptr, new_count)                                    \
    ((type *)(fl_reallocate(arena, ptr, new_count, SIZEOF(type), ALIGNOF(type))))
