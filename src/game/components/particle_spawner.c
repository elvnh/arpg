...
b32 particle_spawner_is_finished(ParticleSpawner *ps)
{
    ...
    b32 result =
        !has_flag(ps->config.flags, PS_FLAG_INFINITE) && (ps->particles_left_to_spawn <= 0);
...
void spawn_particles_in_chunk(Chunk *chunk, Rectangle spawn_area, ParticleSpawnerConfig config,
    s32 particle_count)
{
...
void update_particle_spawner(World *world, Entity *entity, ParticleSpawner *ps,
